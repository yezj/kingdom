...
class InfoHandler(ApiHandler):
    ...
    @api('Book info', '/book/info/', [
        ...
        ], filters=[ps_filter], description="Book info")
    def post(self):
        uid = self.uid
        user = yield self.get_user(uid)
        buytimes = yield self.redis.get('prodbuytimes:%s' % uid)
        if not buytimes:
            buytimes = 0
            yield self.redis.set('prodbuytimes:%s' % uid, buytimes)
...
class BuyHandler(ApiHandler):
    ...
    @defer.inlineCallbacks
    @utils.signed
    @api('Book buy', '/book/buy/', [
        Param('channel', False, str, 'putaogame', 'putaogame', 'channel'),
        Param('_sign', True, str, 'gGRwMApTJ3VpZCcKcDEKUyc5JwpwMgpzLK', 'gGRwMApTJ3VpZCcKcDEKUyc5JwpwMgpzLK', '_sign'),
        ], filters=[ps_filter], description="Book buy")
    def post(self):
        uid = self.uid
        user = yield self.get_user(uid)
        buytimes = yield self.redis.get('prodbuytimes:%s' % uid)
        if not buytimes:
            buytimes = 0
        maxtimes = E.bookmaxtimes(user['vrock'])
        if buytimes < maxtimes:
            cost = E.buy4book(buytimes)
