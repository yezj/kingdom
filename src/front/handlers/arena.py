...
class InfoHandler(ApiHandler):
    ...
    def post(self):
        ...
        if arenas:
            ...
            cuser = dict(arena_coin=arenas['arena_coin'], guards=arenas['guards'].keys(), competitor=competitor, \
                        formation=arenas['formation'], now_rank=arenas['now_rank'], before_rank=arenas['before_rank'],\
            ...
            if str(uid) != str(cid):
                if cuser:
                    ...
                    if now_rank < before_rank:
                        before = before_rank
                        up = before - now_rank
                        now_rank, before_rank, last_rank, awards = yield self.update_arenarank(user, now_rank, before_rank, last_rank)
                        ...
                        if now_rank == 1:
                            yield self.redis.rpush("message", pickle.dumps([D.ARENATOPMESSAGE[0] % user['nickname'], D.ARENATOPMESSAGE[1] % int(time.time())]))
                ...
                if arenawintimes >= 3:
                    yield self.redis.rpush("message", pickle.dumps([D.ARENAWINMESSAGE[0] % user['nickname'], D.ARENAWINMESSAGE[1] % int(time.time())]))
                yield self.set_arena(uid)
...
class MatchHandler(ApiHandler):
    ...
    def post(self):
        ...
        if int(result):
            ...
            if cuser:
                now_rank, before_rank, last_rank = yield self.update_arenaresult(user, self.arg('cid'), self.arg('result'))
                if now_rank < before_rank:
                    before = before_rank
                    up = before - now_rank
                    now_rank, before_rank, last_rank, awards = yield self.update_arenarank(user, now_rank, before_rank, last_rank)
                    ...
                    cuser = dict(now_rank=now_rank, before_rank=before, last_rank=last_rank, up=up, awards=awards, mails=cuser['mails'])
                else:
                    cuser = dict(now_rank=now_rank, before_rank=before_rank, last_rank=last_rank)
                nuser = dict(works=user['works'])
        ...
        if res:
            for r in res:
                ...
                if arenas:
                    ...
                    rank[r[1]] = dict(uid=arenas['uid'], guards=guards, now_rank=arenas['now_rank'], nickname=arenas['nickname'], \
...
class RecordHandler(ApiHandler):
    ...
    def post(self):
        ...
        res = yield self.sql.runQuery("SELECT competitor_id, result, ascend, timestamp FROM core_arenaresult WHERE user_id=%s ORDER BY timestamp", (uid, ))
        records = []
        for r in res:
            arenas = yield self.get_arena(r[0])
            cuser = yield self.get_user(r[0])
            ...
            if arenas:
                ...
                records.append(dict(uid=r[0], result=result, ascend=r[2], timestamp=r[3], xp=arenas['xp'], avat=arenas['avat'],\
                 nickname=arenas['nickname'], heros=guards, formation=arenas['formation'], beautys=beautys))
        res = yield self.sql.runQuery("SELECT user_id, result, ascend, timestamp FROM core_arenaresult WHERE competitor_id=%s ORDER BY timestamp", (uid, ))
        for r in res:
            arenas = yield self.get_arena(r[0])
            cuser = yield self.get_user(r[0])
            beautys = yield self.get_beautys(cuser)
            ...
            if arenas:
                ...
                if result == E.true:
                    ...
                    result = E.true
                    ascend = r[2]
                records.append(dict(uid=r[0], result=result, ascend=ascend, timestamp=r[3], xp=arenas['xp'], avat=arenas['avat'],\
                 nickname=arenas['nickname'], heros=guards, formation=arenas['formation'], beautys=beautys))
