...
class InfoHandler(ApiHandler):
    ...
    def post(self):
        ...
        res = yield self.sql.runQuery("SELECT versus_coin, timestamp FROM core_userstate WHERE user_id=%s LIMIT 1",
                                      (uid,))
        if res:
        ...
        ret = dict(user=cuser, versus=versusmark, reward=reward, rate=int(rate.get('versus_coin', 0)),
            ...
            if str(uid) != str(cid):
                if cuser:
                    ...
                    if now_rank < before_rank:
                        before = before_rank
                        up = before - now_rank
                        now_rank, before_rank, last_rank, awards = yield self.update_versusrank(user, now_rank,
                        ...
                        if now_rank == 1:
                            yield self.redis.rpush("message", pickle.dumps(
                                [D.VERSUSTOPMESSAGE[0] % user['nickname'], D.VERSUSTOPMESSAGE[1] % int(time.time())]))
                ...
                if versuswintimes >= 3:
                    yield self.redis.rpush("message", pickle.dumps(
                        [D.VERSUSWINMESSAGE[0] % user['nickname'], D.VERSUSWINMESSAGE[1] % int(time.time())]))
...
class GuardHandler(ApiHandler):
    ...
    def post(self):
        ...
        if (heros1 and positions1 and formation1) and (heros2 and positions2 and formation2):
            yield self.update_versusguard(uid, sid, self.arg('heros1').split(','), self.arg('positions1').split(','),
                                          formation1, \
...
class RecordHandler(ApiHandler):
    ...
    def post(self):
        ...
        query = "SELECT competitor_id, result, ascend, timestamp FROM %s" % "".join(
            [D.VERSUSSTATE[sid], "result"]) + " WHERE user_id=%s ORDER BY timestamp"
        res = yield self.sql.runQuery(query, (uid,))
        records = []
        for r in res:
            versus = yield self.get_versus(r[0], sid)
        ...
        query = "SELECT user_id, result, ascend, timestamp FROM %s" % "".join(
            [D.VERSUSSTATE[sid], "result"]) + " WHERE competitor_id=%s ORDER BY timestamp"
        res = yield self.sql.runQuery(query, (uid,))
        for r in res:
            versus = yield self.get_versus(r[0], sid)
            cuser = yield self.get_user(r[0])
